...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/detroit_recent \
  -H 'Content-Type: application/json' \
  -d '{"dataset":"crime","limit":20}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/detroit_recent`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "data-detroit": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-data-detroit"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-data-detroit
```
